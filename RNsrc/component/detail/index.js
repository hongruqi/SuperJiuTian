...
export default class Detail extends Component {
	...
	catchEvent = event => {
		try{
			...
			else
			{
				...
				this.props.navigation.navigate('ViewImg', {img_arr:this.state.img, info: info })
			}
		}catch(err){
	...
	render(){
		...
		const headHtml = '<h3 id="artileTitle">'+detailData.title+
						'</h3><p id="artileSource">'+this.state.articleSource+' '+this.state.articleTime+'</p>'+'<head><style type="text/css">html,body{overflow-x:hidden;}</style></head>';

		// headHtml = headHtml + '<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=0"><style type="text/css">html,body{width:100%;height:100%;overflow:hidden;}</style></head>';
